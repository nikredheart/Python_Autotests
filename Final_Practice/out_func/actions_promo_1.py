...
def action_promo_1(self, browser):
    ...
    with allure.step('Заполнено поле для промокода'):
        browser.find_element(By.CSS_SELECTOR, '[name="coupon_code"]').send_keys('GIVEMEHALYAVA')
    logging.info('Заполнено поле для промокода')
    time.sleep(2)
    ...
    with allure.step('Проверка скидки'):
        assert '95,00' in promo_number.text
    logging.info('Проверка скидки')

    total_sum = browser.find_element(By.XPATH, '(//*[@class="woocommerce-Price-amount amount"])[7]')
    with allure.step('Проверка суммы'):
        assert '855,00' in total_sum.text
    ...
    with allure.step('Проверка сообщения'):
        assert 'Coupon code applied successfully.' in message.text
