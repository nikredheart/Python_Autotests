...
def action_promo_4(self, browser):
    ...
    with allure.step('Заполнено поле для промокода (ранее применявшийся промокод)'):
        browser.find_element(By.CSS_SELECTOR, '[name="coupon_code"]').send_keys('GIVEMEHALYAVA')
    logging.info('Заполнено поле для промокода (ранее применявшийся промокод)')
    time.sleep(2)
    ...
    with allure.step('Проверка сообщения'):
        assert 'The promo code has already been applied.' in message.text
